...
# Selecting a ee$ImageCollection. Problem: map seems to be only displaying a small portion of world data; unsure why this is the case. 
...
Map$addLayer(img,visParams=list(bands = "NO2_column_number_density", min = 0,max = 0.0002, palette = c("black", "blue", "purple", "cyan", "green", "yellow", "red")), "True Color Image")
```
...
```{r, eval = TRUE}
NO2img <- ee$ImageCollection("COPERNICUS/S5P/NRTI/L3_NO2")$filterDate('2020-01-01', '2020-04-01')$select("NO2_column_number_density") %>%
  ee$ImageCollection$mean()
pal<-c('#000000','#0000FF','#00FF00','#FF0000')
Map$setCenter(2.373,48.8683,8)
map.zones<-NO2img$expression("(b('NO2_column_number_density') > 0.00015) ? 3 :
(b('NO2_column_number_density') > 0.0001) ? 2 : (b('NO2_column_number_density') > 0.00005) ? 1 : 0"
)
Map$addLayer(map.zones,list(min=0,max=3,palette=pal,opacity=0.4),
 'NO2 concentration')
```
