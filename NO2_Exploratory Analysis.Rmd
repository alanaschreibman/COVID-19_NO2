...
#Continent data using cloud storage. 
image1 <- ee$ImageCollection("COPERNICUS/S5P/NRTI/L3_NO2")$filterDate('2018-07-01', '2018-10-01')$select("NO2_column_number_density") %>%
   ee$ImageCollection$mean()
geometry <- ee$Geometry$Rectangle(
  coords = c(-180, 5,-9, 90),
...
NO2.NorthAmerica <- ee_image_to_gcs(
  image = image1,
  region = geometry,
  bucket = "rgee_bucket", 
  fileFormat = "GEO_TIFF",
  fileNamePrefix = "NO2_NorthAmerica_earthengine"
)
NO2.NorthAmerica$start()
ee_monitoring(NO2.NorthAmerica)

#All countries using cloud storage: making into function where coordinates are input. 
countries.NO2.fxn <- function(minx, miny, maxx, maxy, country) {
  image1 <- ee$ImageCollection("COPERNICUS/S5P/NRTI/L3_NO2")$filterDate('2018-07-10', '2019-07-10')$select("NO2_column_number_density") %>% ee$ImageCollection$mean()
  geometry <- ee$Geometry$Rectangle(
  ...
  NO2.world.countries <- ee_image_to_gcs(
    ...
    bucket = "rgee_bucket", 
    fileFormat = "GEO_TIFF",
    fileNamePrefix = paste0((country), "_NO2")
  )
...
#Applying function:
US.date.fxn("2018-07-01", "2018-10-01") #Used for ground data to satellite comparison. 
