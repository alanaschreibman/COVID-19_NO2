...
NO2 <- ee_as_raster(
  ...
  region = geometry,
  via = "gcs",
  container = "rgee_bucket"
)

#World data using cloud storage. Commands seem to work; problem is that only small regions can be downloaded at a time because there are too many pixels.
image1 <- ee$ImageCollection("COPERNICUS/S5P/NRTI/L3_NO2")$filterDate('2018-07-01', '2018-10-01')$select("NO2_column_number_density") %>%
   ee$ImageCollection$mean()
geometry <- ee$Geometry$Rectangle(
  coords = c(-179, -89, 180, 90),
  scale = 30,
  crs = 'EPSG:4326'
)
NO2.world <- ee_image_to_gcs(
  image = image1,
  region = geometry,
  bucket = "rgee_bucket", 
  fileFormat = "GEO_TIFF",
  fileNamePrefix = "NO2_world_earthengine"
)
NO2.world$start()
ee_monitoring(NO2.world)

#Continental US only using cloud storage. 
image1 <- ee$ImageCollection("COPERNICUS/S5P/NRTI/L3_NO2")$filterDate('2018-07-01', '2018-10-01')$select("NO2_column_number_density") %>%
   ee$ImageCollection$mean()
geometry <- ee$Geometry$Rectangle(
...
NO2.continental.US <- ee_image_to_gcs(
  ...
  bucket = "rgee_bucket", 
  fileFormat = "GEO_TIFF",
  fileNamePrefix = "NO2_earthengine2"
)
NO2.continental.US$start()
ee_monitoring(NO2.continental.US)
...
date.fxn <- function(date.start, date.end) {
  image1 <- ee$ImageCollection("COPERNICUS/S5P/NRTI/L3_NO2")$filterDate(date.start, date.end)$select("NO2_column_number_density") %>% ee$ImageCollection$mean()
  NO2.continental.US <- ee_image_to_gcs(
    image = image1,
    region = geometry,
    bucket = "rgee_bucket", 
...
#Applying function:
#Go by first day of first lockdown order -> two months later. Then find time period of equivalent two-month length, starting before the lockdown. Apply this first to 2019, then 2020, then 2021. 

#United States: California on March 19th. 
date.fxn("2018-07-01", "2018-10-01")

date.fxn("2019-01-19", "2019-03-19") 
date.fxn("2019-03-19", "2019-05-19") 

date.fxn("2020-01-19", "2020-03-19") 
date.fxn("2020-03-19", "2020-05-19") 

date.fxn("2021-01-19", "2021-03-19") 
date.fxn("2021-03-19", "2021-05-19") 
```
