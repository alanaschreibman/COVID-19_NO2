...
#Attempt to use cloud storage: different method. 
image1 <- ee$ImageCollection("COPERNICUS/S5P/NRTI/L3_NO2")$filterDate('2018-07-10', '2020-07-10')$select("NO2_column_number_density") %>%
   ee$ImageCollection$mean()
geometry <- ee$Geometry$Rectangle(
...
NO2.earthengine2 <- ee_image_to_gcs(
  image = image1,
  region = geometry,
  bucket = "rgee_bucket", 
  fileFormat = "GEO_TIFF",
...
ee_gcs_to_local(task = NO2.earthengine2)
