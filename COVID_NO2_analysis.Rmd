...
#### World Urban Rural gradient shapefiles
```{r, eval = TRUE}
#lockdown time https://en.wikipedia.org/wiki/COVID-19_lockdowns  
...
#argentina
#israel 
wrld_simpl[wrld_simpl$NAME == "Czechia", ]
top.10.countries <- covid.NO2.df %>% filter(Area > 100) %>% arrange(desc(totcases.mil)) 
top.10.countries <- list(top.10.countries$location[1:10])

---
world.urban.sp <- as(world.sf, Class = "Spatial")
top.10.sp <- gIntersection(wrld_simpl[wrld_simpl$NAME == "United States", ], world.urban.sp)
test <- st_as_sf(test)
---
rastlist.fxn <- function(location) {
  list.files(path = paste0("time_analysis_2019_", location), pattern='.tif$', 
all.files=TRUE, full.names=FALSE) 
}
top.10.urban.raster.list <- purrr:pmap(top.10.countries)


top.10.urban.fxn <- function(location) {
  country.sp <- gIntersection(wrld_simpl[wrld_simpl$NAME == location, ], world.urban.sp)
  country.sf <- st_as_sf(country.sp)
top.10.urban.list <- purrr:pmap(top.10.countries) 
  r.vals <- exact_extract(r, country.sf, weights = 'area', 'mean') 
  ...
  urban.2019.weekly <- cbind(final.df, Location, Period, Date, Week) 
  urban.2019.weekly <- urban.2019.weekly %>% select(Location, Period, Date, Week, NO2_Concentration)
}
#list of rasters for each country with elements for each week 

top.10.urban.list <- purrr:pmap(top.10.countries, ) #creates list of urban shapefiles for each country
```
...
dates.2020.mod <- seq(as.Date("2018-12-16"), as.Date("2018-12-29"), by = 7)
#Function to make data frame for weekly average NO2 concentration. 
raster.extract.fxn <- function(x, y) {
...
# read data
head(us.urban.weekly.df)

#main plot
year_scatter_plot <- function(data,ylim,xlockdown,pollutant,model){
  datanew <- data.frame(pollution_estimate=mean(data$NO2_Concentration),covid_lockdown=rep(c(0,1,0,1),c(120,410,120,410)), no_of_weeks=c((1:530/10), (1:530/10)), year2020 = rep(c(0,1),c(530,530)))
  ...
  pred1 <- predict(model,type="response",datanew) #/mean(data$pollution_estimate)*10
  ...
  plot(NO2_Concentration ~ no_of_weeks, data = data.2019,
       type="n",ylim=c(00,ylim),xlab="Weeks", ylab=paste0(pollutant," estimates"),
       bty="l")
  rect(xlockdown,0,53,ylim,col=grey(0.9),border=F)
  points(NO2_Concentration ~ no_of_weeks,data = data.2019,cex=0.7,pch=16,col=alpha(col[1],0.5))
  legend(47, 0.5E-5, legend=c("2019", "2020"), col=alpha(col,0.5),pch=16,cex=0.7)
  
  #plot for 2020
   data.2020 <- data %>% filter(., year2020 == 1) %>% select(., NO2_Concentration, no_of_weeks) 
  par(new=TRUE)
  plot(NO2_Concentration ~ no_of_weeks,data = data.2020, type="n",ylim=c(00,ylim),xlab="Weeks", ylab=paste0(pollutant," estimates"), bty="l")
  points(NO2_Concentration ~ no_of_weeks,data = data.2020,cex=0.7,pch=16,col=alpha(col[2],0.5))
  #Add prediction lines
  lines(datanew$no_of_weeks[1:530],pred1[1:530],col=col[1],lwd = 1.5)
  lines(datanew$no_of_weeks[531:1060],pred1[531:1060],col=col[2],lwd = 1.5)
  #legend(91, 29.8, legend=c("2015-2019", "2020"), col=col,lty=1)
  title(paste0(pollutant," for Jan to Dec 2020 vs 2015-19"))
  ...
  # to plot the counterfactual scenario we create a data frame as if covid lockdown (the intervention) was never being implemented
    datanew <- data.frame(pollution_estimate=mean(data$NO2_Concentration),covid_lockdown=0, no_of_weeks=c((1:530/10), (1:530/10)), year2020 = rep(c(0,1),c(530,530)))
    
  # generate predictions under the counterfactual scenario and add it to the plot
  pred1b <- predict(model,datanew,type="response") #/mean(data$pollution_estimate)*10
  lines(datanew$no_of_weeks[1:530],pred1b[1:530],lty=2,col=col[1],lwd = 1.5)
  lines(datanew$no_of_weeks[531:1060],pred1b[531:1060],col=col[2],lty=2,lwd = 1.5)
}

#Data for NO2
data.NO2.19 <- us.urban.weekly.df %>%
            dplyr::filter(Period == 2019) %>%
            dplyr::mutate(covid_lockdown = rep(c(0,1),c(12,40)),year2020 = 0,no_of_weeks = 1:52) 

data.NO2.19[nrow(data.NO2.19)+1,] <- NA
data.NO2.19[53,8] = 53
data.NO2.19[53,6] = 1
data.NO2.19[53,7] = 0

data.NO2.20 <- us.urban.weekly.df %>%
            dplyr::filter(Period == 2020) %>%
            dplyr::mutate(covid_lockdown = rep(c(0,1),c(12,41)),year2020 = 1,no_of_weeks = seq(1:53))
          

data.NO2 <- rbind(data.NO2.19,data.NO2.20)
#data.NO2$no_of_weeks <- seq(1:nrow(data.NO2))

#tabulate pollution before and after lockdown
summary(data.NO2)
summary(data.NO2$NO2_Concentration[data.NO2$covid_lockdown==0])
summary(data.NO2$NO2_Concentration[data.NO2$covid_lockdown==1])

#OLS regression
year.model.us.urban <- lm(NO2_Concentration ~ no_of_weeks*year2020 + covid_lockdown*year2020 + no_of_weeks*covid_lockdown*year2020 +  harmonic(no_of_weeks,2,52), data=data.NO2) 
#year.model.us.urban <- lm(NO2_Concentration ~ no_of_weeks*year2020 + covid_lockdown*year2020 + no_of_weeks*covid_lockdown*year2020, data=data.NO2) 
summary(year.model.us.urban)

#plot
year_scatter_plot(data.NO2,ylim=5E-5, xlockdown=12,pollutant="NO2",model=year.model.us.urban)
```

#### ITS analysis: US urban Dec-June
```{r, eval = TRUE}
# Install packages required for the analysis (uncomment if needed)
#install.packages("lmtest") ; install.packages("Epi")
#install.packages("tsModel"); install.packages("vcd")
...
data.NO2.19 <- us.urban.weekly.df %>%
            dplyr::filter(Period == 2019) 
data.NO2.19 <- data.NO2.19[data.NO2.19$Date >= "2019-12-17" & data.NO2.19$Date <= "2020-06-10", ] %>%
  dplyr::mutate(covid_lockdown =  rep(c(0,1),c(13,13)),year2020 = 0,no_of_weeks = 1:26) 
data.NO2.19[nrow(data.NO2.19)+1,] <- NA
data.NO2.19[27,8] = 27
data.NO2.19[27,6] = 1
data.NO2.19[27,7] = 0
...
data.NO2.20 <- us.urban.weekly.df %>%
            dplyr::filter(Period == 2020) 
data.NO2.20 <- data.NO2.20[data.NO2.20$Date >= "2019-12-16" & data.NO2.20$Date <= "2020-06-15", ] %>%
    dplyr::mutate(covid_lockdown = rep(c(0,1),c(13,14)),year2020 = 1,no_of_weeks = seq(1:27))
...
scatter_gplot(data.NO2, 5E-5, 13, "Urban U.S.")

#main plot
period_scatter_plot <- function(data,ylim,xlockdown,location,model){
    datanew <- data.frame(NO2_Concentration=mean(data$NO2_Concentration),covid_lockdown=rep(c(0,1,0,1),c(130,140,130,140)), no_of_weeks=c((1:270/10), (1:270/10)), year2020 = rep(c(0,1),c(270, 270)))
  ...
  plot(NO2_Concentration ~ no_of_weeks, data = data.2019,
       type="n",ylim=c(00,ylim),xlab="Weeks", ylab=paste0("NO2 estimates"),
       bty="l")
  rect(xlockdown,0,27,ylim,col=grey(0.9),border=F)
  points(NO2_Concentration ~ no_of_weeks,data = data.2019,cex=0.7,pch=16,col=alpha(col[1],0.5))
  legend(19, 0.5E-5, legend=c("2019", "2020"), col=alpha(col,0.5),pch=16,cex=0.7)
  
  #plot for 2020
   data.2020 <- data %>% filter(., year2020 == 1) %>% select(., NO2_Concentration, no_of_weeks) 
  par(new=TRUE)
  plot(NO2_Concentration ~ no_of_weeks,data = data.2020, type="n",ylim=c(00,ylim),xlab="Weeks", ylab=paste0("NO2 estimates"), bty="l")
  points(NO2_Concentration ~ no_of_weeks,data = data.2020,cex=0.7,pch=16,col=alpha(col[2],0.5))
  #Add prediction lines
  lines(datanew$no_of_weeks[1:270],pred1[1:270],col=col[1],lwd = 1.5) #regular model, 2019
  lines(datanew$no_of_weeks[271:540],pred1[271:540],col=col[2],lwd = 1.5) #regular model, 2020
  #legend(91, 29.8, legend=c("2015-2019", "2020"), col=col,lty=1)
  title(paste(location,"NO2, 19th Dec to 19th June 2020 vs 2019"))

  # to plot the counterfactual scenario we create a data frame as if covid lockdown (the intervention) was never being implemented
    datanew.ctr <- data.frame(NO2_Concentration=mean(data$NO2_Concentration),covid_lockdown=0, no_of_weeks=c((1:270/10), (1:270/10)), year2020 = rep(c(0,1),c(270, 270)))

  # generate predictions under the counterfactual scenario and add it to the plot
  pred1b <- predict(model,datanew.ctr, type="response") #/mean(data$pollution_estimate)*10
   
  lines(datanew.ctr$no_of_weeks[1:270],pred1b[1:270],col=col[1],lty=2,lwd = 1.5)
  lines(datanew.ctr$no_of_weeks[271:540],pred1b[271:540],col=col[2],lty=2,lwd = 1.5)
}
...
#OLS regression
#period.model.us.urban <- lm(NO2_Concentration ~ no_of_weeks*year2020 + covid_lockdown*year2020 + no_of_weeks*covid_lockdown*year2020 +  harmonic(no_of_weeks,2,52), data=data.NO2) 
period.model.us.urban <- lm(NO2_Concentration ~ no_of_weeks*year2020 + covid_lockdown*year2020 + no_of_weeks*covid_lockdown*year2020, data=data.NO2) 
summary(period.model.us.urban)

#plot
period_scatter_plot(data.NO2,ylim=5E-5, xlockdown=13,location="Urban U.S.",model=period.model.us.urban)
```

#### ITS analysis: US rural Jan-Dec. 
```{r, eval = TRUE}
#Data for NO2
rural.data.NO2.19 <- us.rural.weekly.df %>%
            dplyr::filter(Period == 2019) %>%
            dplyr::mutate(covid_lockdown = 0,year2020 = 0,no_of_weeks = 1:52) 

rural.data.NO2.19[nrow(rural.data.NO2.19)+1,] <- NA
rural.data.NO2.19[53,8] = 53
rural.data.NO2.19[53,7] = 0

rural.data.NO2.20 <- us.rural.weekly.df %>%
            dplyr::filter(Period == 2020) %>%
            dplyr::mutate(covid_lockdown = rep(c(0,1),c(12,41)),year2020 = 1,no_of_weeks = seq(1:53))
          

rural.data.NO2 <- rbind(rural.data.NO2.19,rural.data.NO2.20)
#data.NO2$no_of_weeks <- seq(1:nrow(data.NO2))

#tabulate pollution before and after lockdown
summary(rural.data.NO2)
summary(rural.data.NO2$NO2_Concentration[rural.data.NO2$covid_lockdown==0])
summary(rural.data.NO2$NO2_Concentration[rural.data.NO2$covid_lockdown==1])

#OLS regression
year.model.us.rural <- lm(NO2_Concentration ~ no_of_weeks*year2020 + covid_lockdown*year2020 + no_of_weeks*covid_lockdown*year2020 +  harmonic(no_of_weeks,2,52), data=rural.data.NO2) 
#year.model.us.rural <- lm(NO2_Concentration ~ no_of_weeks*year2020 + covid_lockdown*year2020 + no_of_weeks*covid_lockdown*year2020, data=rural.data.NO2) 
summary(year.model.us.rural)

#plot
year_scatter_plot(rural.data.NO2,ylim=5E-5, xlockdown=12,location="Rural U.S.",model=year.model.us.rural)
```

### ITS analysis: US rural Dec-June
```{r, eval = TRUE}
scatter_gplot(rural.data.NO2, 5E-5, 13, "Rural U.S.")

#Data for NO2
rural.data.NO2.19 <- us.rural.weekly.df %>%
            dplyr::filter(Period == 2019) 
rural.data.NO2.19 <- rural.data.NO2.19[rural.data.NO2.19$Date >= "2019-12-17" & rural.data.NO2.19$Date <= "2020-06-10", ] %>%
  dplyr::mutate(covid_lockdown =  rep(c(0,1),c(13,13)),year2020 = 0,no_of_weeks = 1:26) 
rural.data.NO2.19[nrow(rural.data.NO2.19)+1,] <- NA
rural.data.NO2.19[27,8] = 27
rural.data.NO2.19[27,6] = 1
rural.data.NO2.19[27,7] = 0

rural.data.NO2.20 <- us.rural.weekly.df %>%
            dplyr::filter(Period == 2020) 
rural.data.NO2.20 <- rural.data.NO2.20[rural.data.NO2.20$Date >= "2019-12-16" & rural.data.NO2.20$Date <= "2020-06-15", ] %>%
    dplyr::mutate(covid_lockdown = rep(c(0,1),c(13,14)),year2020 = 1,no_of_weeks = seq(1:27))
...
#OLS regression
#period.model.us.rural <- lm(NO2_Concentration ~ no_of_weeks*year2020 + covid_lockdown*year2020 + no_of_weeks*covid_lockdown*year2020 +  harmonic(no_of_weeks,2,52), data=data.NO2) 
period.model.us.rural <- lm(NO2_Concentration ~ no_of_weeks*year2020 + covid_lockdown*year2020 + no_of_weeks*covid_lockdown*year2020, data=rural.data.NO2) 
summary(period.model.us.rural)

#plot
period_scatter_plot(rural.data.NO2,ylim=5E-5, xlockdown=13,location="Rural U.S.",model=period.model.us.rural)
```
