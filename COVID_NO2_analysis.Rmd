...
world.rural.sf <- st_as_sf(world.rural.sp)
world.rural.sf <- st_transform(world.rural.sf, "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")  
```
...
purrr::pmap(list(stringencyindex.split.df[[9]]$lockdown.date, top3.urban.shp.list[rm.nkorea], top3.rural.shp.list[rm.nkorea], top3.list[rm.nkorea], "stringencyindex"), write.csv.fxn)

--- #DELETE 
date.indexes <- date.index.fxn("2020-01-30")
#2019 urban
r.2019 <- weekly.list.2019[date.indexes]
shape <- rep(world.urban.sf, length.out = length(r.2019))
date.2019 <- dates.2019[date.indexes]
urban.2019.weekly.df <- purrr::pmap_dfr(list(r.2019, shape, date.2019), world.extract.2019.fxn)
#2020 urban
r.2020 <- weekly.list.2020[date.indexes]
...
#2019 rural
shape <- rep(world.rural.sf, length.out = length(r.2019))
rural.2019.weekly.df <- purrr::pmap_dfr(list(r.2019, shape, date.2019), world.extract.2019.fxn)
#2020 rural
rural.2020.weekly.df <- purrr::pmap_dfr(list(r.2020, shape, date.2020), world.extract.2020.fxn)
rural.weekly.df <- rbind(rural.2019.weekly.df, rural.2020.weekly.df) 
