...
country.owid.list <- list(owid.countries$location)
country.owid.list.shp <- list(paste0(owid.countries$location, ".shp"))
...
extraction.fxn <- function(x) {
  ...
  names(final.df)[11] <- "Latitude" 
  names(final.df)[12] <- "NO2_Concentration"
  final.df <- final.df %>% select(location, Area, Longitude, Latitude, NO2_Concentration)
}

#Changing shapefile names to match COVID dataset:
Brunei.shp <- `Brunei Darussalam.shp`
  Brunei.shp$NAME <- as.character(Brunei.shp$NAME)
  Brunei.shp@data[1,5] <- sub("Brunei Darussalam", "Brunei", Brunei.shp@data[1,5])
  rm(`Brunei Darussalam.shp`)
Czechia.shp <- `Czech Republic.shp`
  Czechia.shp$NAME <- as.character(Czechia.shp$NAME)
  Czechia.shp@data[1,5] <- sub("Czech Republic", "Czechia", Czechia.shp@data[1,5])
  rm(`Czech Republic.shp`)
`Democratic Republic of Congo.shp` <- `Democratic Republic of the Congo.shp`
  `Democratic Republic of Congo.shp`$NAME <- as.character(`Democratic Republic of Congo.shp`$NAME)
  `Democratic Republic of Congo.shp`@data[1,5] <- sub("Democratic Republic of the Congo", "Democratic Republic of Congo", `Democratic Republic of Congo.shp`@data[1,5])
  rm(`Democratic Republic of the Congo.shp`)
`Faeroe Islands.shp` <- `Faroe Islands.shp`
  `Faeroe Islands.shp`@data[1,5] <- sub("Faroe Islands.shp", "Faeroe Islands.shp", `Faeroe Islands.shp`@data[1,5])
  rm(`Faroe Islands.shp`)
Iran.shp <- `Iran (Islamic Republic of).shp`
  Iran.shp$NAME <- as.character(Iran.shp$NAME) 
  Iran.shp@data$NAME[Iran.shp@data$NAME == "Iran (Islamic Republic of)"] <- "Iran"
  rm(`Iran (Islamic Republic of).shp`)
Laos.shp <-  `Lao People's Democratic Republic.shp`
  Laos.shp$NAME <- as.character(Laos.shp$NAME)
  Laos.shp@data[1,5] <- sub("Lao People's Democratic Republic", "Laos", Laos.shp@data[1,5])
  rm(`Lao People's Democratic Republic.shp`)
Libya.shp <- `Libyan Arab Jamahiriya.shp`
  Libya.shp$NAME <- as.character(Libya.shp$NAME) 
  Libya.shp@data[1,5] <- sub("Libyan Arab Jamahiriya", "Libya", Libya.shp@data[1,5])
  rm(`Libyan Arab Jamahiriya.shp`)
Moldova.shp <- `Republic of Moldova.shp`
  Moldova.shp$NAME <- as.character(Moldova.shp$NAME) 
  Moldova.shp@data[1,5] <- sub("Republic of Moldova", "Moldova", Moldova.shp@data[1,5])
  rm(`Republic of Moldova.shp`)
Myanmar.shp <- Burma.shp
  Myanmar.shp$NAME <- as.character(Myanmar.shp$NAME) 
  Myanmar.shp@data[1,5] <- sub("Burma", "Myanmar", Myanmar.shp@data[1,5])
  rm(Burma.shp)
`South Korea.shp` <- `Korea, Republic of.shp`
  `South Korea.shp`$NAME <- as.character(`South Korea.shp`$NAME) 
  `South Korea.shp`@data[1,5] <- sub("Korea, Republic of", "South Korea", `South Korea.shp`@data[1,5])
  rm(`Korea, Republic of.shp`)
`North Korea.shp` <- `Korea, Democratic People's Republic of.shp`
  `North Korea.shp`$NAME <- as.character(`North Korea.shp`$NAME) 
  `North Korea.shp`@data[1,5] <- sub("Korea, Democratic People's Republic of", "North Korea",`North Korea.shp`@data[1,5])
  rm(`Korea, Democratic People's Republic of.shp`)
`Syria.shp` <- `Syrian Arab Republic.shp`
  Syria.shp$NAME <- as.character(Syria.shp$NAME) 
  Syria.shp@data[1,5] <- sub("Syrian Arab Republic", "Syria", Syria.shp@data[1,5])
  rm(`Syrian Arab Republic.shp`)
`North Macedonia.shp` <- `The former Yugoslav Republic of Macedonia.shp`
  `North Macedonia.shp`$NAME <- as.character(`North Macedonia.shp`$NAME) 
  `North Macedonia.shp`@data[1,5] <- sub("The former Yugoslav Republic of Macedonia", "North Macedonia", `North Macedonia.shp`@data[1,5])
  rm(`The former Yugoslav Republic of Macedonia.shp`)
Macao.shp <- Macau.shp
  Macao.shp$NAME <- as.character(Macao.shp$NAME) 
  Macao.shp@data[1,5] <- sub("Macao", "Macao", Macao.shp@data[1,5])
  rm(`Macau.shp`)
`Micronesia (country).shp` <- `Micronesia, Federated States of.shp`
  `Micronesia (country).shp`$NAME <- as.character(`Micronesia (country).shp`$NAME) 
  `Micronesia (country).shp`@data$NAME[`Micronesia (country).shp`@data$NAME == "Micronesia, Federated States of"] <- "Micronesia (country)"  
  rm(`Micronesia, Federated States of.shp`)
Timor.shp <- `Timor-Leste.shp`
  Timor.shp$NAME <- as.character(Timor.shp$NAME) 
  Timor.shp@data[1,5] <- sub("Timor-Leste", "Timor", Timor.shp@data[1,5])
  rm(`Timor-Leste.shp`)
Vatican.shp <- `Holy See (Vatican City).shp`
  Vatican.shp$NAME <- as.character(Vatican.shp$NAME) 
  Vatican.shp@data$NAME[Vatican.shp@data$NAME == "Holy See (Vatican City)"] <- "Vatican"  
  rm(`Holy See (Vatican City).shp`)
Vietnam.shp <- `Viet Nam.shp`
  Vietnam.shp$NAME <- as.character(Vietnam.shp$NAME) 
  Vietnam.shp@data[1,5] <- sub("Viet Nam", "Vietnam", Vietnam.shp@data[1,5])
  rm(`Viet Nam.shp`)
`Wallis and Futuna.shp` <- `Wallis and Futuna Islands.shp`
  `Wallis and Futuna.shp`$NAME <- as.character(`Wallis and Futuna.shp`$NAME) 
  `Wallis and Futuna.shp`@data[1,5] <- sub("Wallis and Futuna Islands.shp", "Wallis and Futuna.shp", `Wallis and Futuna.shp`@data[1,5])
  rm(`Wallis and Futuna Islands.shp`)

#Extracting mean NO2 values and adding to coordinates data to make final data frame.
  #Missing from shapefiles: Kosovo, Tanzania, Eswatini
total.df = data.frame()
for (i in c(Afghanistan.shp, Albania.shp, Algeria.shp, Andorra.shp, Angola.shp, `Antigua and Barbuda.shp`, Argentina.shp, Armenia.shp, Aruba.shp, Australia.shp, Austria.shp, Azerbaijan.shp, Bahamas.shp, Bahrain.shp, Bangladesh.shp, Barbados.shp, Belarus.shp, Belgium.shp, Belize.shp, Benin.shp, Bermuda.shp, Bhutan.shp, Bolivia.shp, `Bosnia and Herzegovina.shp`, Botswana.shp, Brazil.shp, Brunei.shp, Bulgaria.shp, `Burkina Faso.shp`, Burundi.shp, Cambodia.shp, Cameroon.shp, Canada.shp, `Cape Verde.shp`, `Cayman Islands.shp`, `Central African Republic.shp`, Chad.shp, Chile.shp, China.shp, Colombia.shp, Comoros.shp, Congo.shp, `Cook Islands.shp`, `Costa Rica.shp`, `Cote d'Ivoire.shp`, Croatia.shp, Cuba.shp, Cyprus.shp, Czechia.shp, `Democratic Republic of Congo.shp`, Denmark.shp, Djibouti.shp, Dominica.shp, `Dominican Republic.shp`, Ecuador.shp, Egypt.shp, `El Salvador.shp`, `Equatorial Guinea.shp`, Eritrea.shp, Estonia.shp, Ethiopia.shp, `Faeroe Islands.shp`, Fiji.shp, Finland.shp, France.shp, `French Polynesia.shp`, Gabon.shp, Gambia.shp, Georgia.shp, Germany.shp, Ghana.shp, Gibraltar.shp, Greece.shp, Grenada.shp, Guatemala.shp, Guernsey.shp, Guinea.shp, `Guinea-Bissau.shp`, Guyana.shp, Haiti.shp, Honduras.shp, `Hong Kong.shp`, Hungary.shp, Iceland.shp, India.shp, Indonesia.shp, Iran.shp, Iraq.shp, Ireland.shp, `Isle of Man.shp`, Israel.shp, Italy.shp, Jamaica.shp, Japan.shp, Jordan.shp, Kazakhstan.shp, Kenya.shp, Kiribati.shp, Kuwait.shp, Kyrgyzstan.shp, Laos.shp, Latvia.shp, Lebanon.shp, Lesotho.shp, Liberia.shp, Libya.shp, Liechtenstein.shp, Lithuania.shp, Luxembourg.shp, Macao.shp, Madagascar.shp, Malawi.shp, Malaysia.shp, Maldives.shp, Mali.shp, Malta.shp, `Marshall Islands.shp`, Mauritania.shp, Mauritius.shp, Mexico.shp, `Micronesia (country).shp`, Moldova.shp, Monaco.shp, Mongolia.shp, Montenegro.shp, Morocco.shp, Mozambique.shp, Myanmar.shp, Namibia.shp, Nepal.shp, Netherlands.shp, `New Zealand.shp`, Nicaragua.shp, Niger.shp, Nigeria.shp, `North Macedonia.shp`, Norway.shp, Oman.shp, Pakistan.shp, Palestine.shp, Panama.shp, `Papua New Guinea.shp`, Paraguay.shp, Peru.shp, Philippines.shp, Poland.shp, Portugal.shp, Qatar.shp, Romania.shp, Russia.shp, Rwanda.shp, `Saint Kitts and Nevis.shp`, `Saint Lucia.shp`, `Saint Vincent and the Grenadines.shp`, Samoa.shp, `San Marino.shp`, `Sao Tome and Principe.shp`, `Saudi Arabia.shp`, Senegal.shp, Serbia.shp, Seychelles.shp, `Sierra Leone.shp`, Singapore.shp, Slovakia.shp, Slovenia.shp, `Solomon Islands.shp`, Somalia.shp, `South Africa.shp`, `South Korea.shp`, `South Sudan.shp`, Spain.shp, `Sri Lanka.shp`, Sudan.shp, Suriname.shp, Sweden.shp, Switzerland.shp, Syria.shp, Taiwan.shp, Tajikistan.shp, Thailand.shp, Timor.shp, Togo.shp, `Trinidad and Tobago.shp`, Tunisia.shp, Turkey.shp, Uganda.shp, Ukraine.shp, `United Arab Emirates.shp`, `United Kingdom.shp`, `United States.shp`, Uruguay.shp, Uzbekistan.shp, Vanuatu.shp, Vatican.shp, Venezuela.shp, Vietnam.shp, `Wallis and Futuna.shp`, Yemen.shp, Zambia.shp, Zimbabwe.shp)) {
    # vector output
    country.df <- extraction.fxn(i) 
    # add vector to premade dataframe
    df <- data.frame(country.df)
...
#Read in IHME regions data. 
IHME.regions.df <- read.csv(file = 'IHME_regions.csv', header = TRUE) 
IHME.regions.df <- IHME.regions.df %>% rename(location = Countries) %>% select("Super.region", "Regions", "location")

#Merged geospatial data set with COVID data (variables come from COVID exploratory analysis RMD file). 
covid.NO2.df <- right_join(IHME.regions.df, owid.countries, by = "location")
covid.NO2.df <- full_join(covid.NO2.df, total.df, by = "location")
covid.NO2.df$Regions <- as.factor(covid.NO2.df$Regions)
covid.NO2.df$Super.region <- as.factor(covid.NO2.df$Super.region)
```
...
pop.weighting.fxn <- function(x) {
  pop.cropped <- crop(pop.den.r, extent(x))
  pop.cropped <- mask(pop.cropped, x)
  NO2.cropped <- crop(world.NO2.r, extent(x))
  NO2.cropped <- mask(NO2.cropped, x)
  overlay.cropped <- overlay(pop.cropped, NO2.cropped, fun=function(x,y){(x*y)} )
  sum <- cellStats(overlay.cropped, stat='sum')
  mod.covid.df <- filter(covid.NO2.df, location == x$NAME)
  ...
  weighted.NO2.df <- weighted.NO2.df %>% select(location, `Population weighted NO2`)
} #Function to weight NO2 by population for each country. 
  
weighted.NO2.df = data.frame()
for (i in c(Afghanistan.shp, Albania.shp, Algeria.shp, Andorra.shp, Angola.shp, `Antigua and Barbuda.shp`, Argentina.shp, Armenia.shp, Aruba.shp, Australia.shp, Austria.shp, Azerbaijan.shp, Bahamas.shp, Bahrain.shp, Bangladesh.shp, Barbados.shp, Belarus.shp, Belgium.shp, Belize.shp, Benin.shp, Bermuda.shp, Bhutan.shp, Bolivia.shp, `Bosnia and Herzegovina.shp`, Botswana.shp, Brazil.shp, Brunei.shp, Bulgaria.shp, `Burkina Faso.shp`, Burundi.shp, Cambodia.shp, Cameroon.shp, Canada.shp, `Cape Verde.shp`, `Cayman Islands.shp`, `Central African Republic.shp`, Chad.shp, Chile.shp, China.shp, Colombia.shp, Comoros.shp, Congo.shp, `Cook Islands.shp`, `Costa Rica.shp`, `Cote d'Ivoire.shp`, Croatia.shp, Cuba.shp, Cyprus.shp, Czechia.shp, `Democratic Republic of Congo.shp`, Denmark.shp, Djibouti.shp, Dominica.shp, `Dominican Republic.shp`, Ecuador.shp, Egypt.shp, `El Salvador.shp`, `Equatorial Guinea.shp`, Eritrea.shp, Estonia.shp, Ethiopia.shp, `Faeroe Islands.shp`, Fiji.shp, Finland.shp, France.shp, `French Polynesia.shp`, Gabon.shp, Gambia.shp, Georgia.shp, Germany.shp, Ghana.shp, Gibraltar.shp, Greece.shp, Grenada.shp, Guatemala.shp, Guernsey.shp, Guinea.shp, `Guinea-Bissau.shp`, Guyana.shp, Haiti.shp, Honduras.shp, `Hong Kong.shp`, Hungary.shp, Iceland.shp, India.shp, Indonesia.shp, Iran.shp, Iraq.shp, Ireland.shp, `Isle of Man.shp`, Israel.shp, Italy.shp, Jamaica.shp, Japan.shp, Jordan.shp, Kazakhstan.shp, Kenya.shp, Kiribati.shp, Kuwait.shp, Kyrgyzstan.shp, Laos.shp, Latvia.shp, Lebanon.shp, Lesotho.shp, Liberia.shp, Libya.shp, Liechtenstein.shp, Lithuania.shp, Luxembourg.shp, Macao.shp, Madagascar.shp, Malawi.shp, Malaysia.shp, Maldives.shp, Mali.shp, Malta.shp, `Marshall Islands.shp`, Mauritania.shp, Mauritius.shp, Mexico.shp, `Micronesia (country).shp`, Moldova.shp, Monaco.shp, Mongolia.shp, Montenegro.shp, Morocco.shp, Mozambique.shp, Myanmar.shp, Namibia.shp, Nepal.shp, Netherlands.shp, `New Zealand.shp`, Nicaragua.shp, Niger.shp, Nigeria.shp, `North Macedonia.shp`, Norway.shp, Oman.shp, Pakistan.shp, Palestine.shp, Panama.shp, `Papua New Guinea.shp`, Paraguay.shp, Peru.shp, Philippines.shp, Poland.shp, Portugal.shp, Qatar.shp, Romania.shp, Russia.shp, Rwanda.shp, `Saint Kitts and Nevis.shp`, `Saint Lucia.shp`, `Saint Vincent and the Grenadines.shp`, Samoa.shp, `San Marino.shp`, `Sao Tome and Principe.shp`, `Saudi Arabia.shp`, Senegal.shp, Serbia.shp, Seychelles.shp, `Sierra Leone.shp`, Singapore.shp, Slovakia.shp, Slovenia.shp, `Solomon Islands.shp`, Somalia.shp, `South Africa.shp`, `South Korea.shp`, `South Sudan.shp`, Spain.shp, `Sri Lanka.shp`, Sudan.shp, Suriname.shp, Sweden.shp, Switzerland.shp, Syria.shp, Taiwan.shp, Tajikistan.shp, Thailand.shp, Timor.shp, Togo.shp, `Trinidad and Tobago.shp`, Tunisia.shp, Turkey.shp, Uganda.shp, Ukraine.shp, `United Arab Emirates.shp`, `United Kingdom.shp`, `United States.shp`, Uruguay.shp, Uzbekistan.shp, Vanuatu.shp, Vatican.shp, Venezuela.shp, Vietnam.shp, `Wallis and Futuna.shp`, Yemen.shp, Zambia.shp, Zimbabwe.shp)) {
  country.wNO2.df <- pop.weighting.fxn(i) 
    # add vector to premade dataframe
  df <- data.frame(country.wNO2.df)
  weighted.NO2.df <- rbind(weighted.NO2.df,df)
} #Apply weighting function to all shapefiles. 
...
map + 
  geom_polygon(aes(fill = NO2_Concentration), color = 'gray', size = 0.1) +
  my_theme() + 
...
map + 
  geom_polygon(aes(fill = Population.weighted.NO2), color = 'gray', size = 0.1) +
  my_theme() + 
...
#Function to make data frame for weekly average NO2 concentration. 
read.fxn <- function(x, y) {
  r <- raster(paste0("China_time_analysis_2019/", x))
...
#Function to make data frame for weekly average NO2 concentration. 
read.fxn <- function(x, y) {
  r <- raster(paste0("China_time_analysis_2020/", x))
  r.vals <- raster::extract(r, `China.shp`)
  r.mean <- lapply(r.vals, FUN=mean, na.rm = TRUE)
  final.df <- data.frame(`China.shp`@data, r.mean)   
  Date <- as.factor(as.Date(dates.2020[[y]]))
