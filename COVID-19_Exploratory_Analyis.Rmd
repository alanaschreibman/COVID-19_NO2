...
covid.mean.fxn <- function(x, column) {
  ...
  df <- data.frame(Date, Week, mean) %>% select(Date, Week, Mean_Weekly_New_Cases = 3)
  df
}

x <- 1:76
mean.newcases.df <- purrr::map2_dfr(x, "newcases", covid.mean.fxn)
mean.newdeaths.df <- purrr::map2_dfr(x, "newdeaths", covid.mean.fxn)
```
