...
index.title.list <- c("school closures", "workplace closures", "cancelled public events", "restrictions on gatherings", "public transport closures", "stay at home orders", "restrictions on internal travel", "restrictions on international travel", "lockdown stringency index")
```
...
#Japan
japan.urban.year.df.list <- purrr::pmap(list("Japan", "urban", file.index.list[c(1:3, 6:8)]), NO2.df.filter.fxn)
japan.rural.year.df.list <- purrr::pmap(list("Japan", "rural", file.index.list[c(1:3, 6:8)]), NO2.df.filter.fxn)

japan.urban.year.plot <- purrr::pmap(list(japan.urban.year.df.list, index.title.list[c(1:3, 6:8)], "Japan urban", "2019-10-31", stringencyindex.split.country.df[[6]]$lockdown.date, "2020-12-24"), ITS.plot.fxn)
japan.rural.year.plot <- purrr::pmap(list(japan.rural.year.df.list, index.title.list[c(1:3, 6:8)], "Japan rural", "2019-10-31", stringencyindex.split.country.df[[6]]$lockdown.date, "2020-12-24"), ITS.plot.fxn)
...
period_scatter_plot <- function(data,ylim,location,lockdown.date,index.type,model){
  ...
  plot(NO2_Concentration ~ no_of_weeks, data = data.2019, type="n",ylim=c(00,ylim),xlab="Date", ylab=expression(paste(NO[2], " ", "estimates, (mol/", m^2, ")")), bty="l", xaxt="n", cex.main=0.5, cex.lab=0.6, cex.axis=0.4, las = 1)
  ...
  plot(NO2_Concentration ~ no_of_weeks,data = data.2020, type="n",ylim=c(00,ylim),xlab="Date", ylab=expression(paste(NO[2], " ", "estimates, (mol/", m^2, ")")), bty="l", xaxt="n", cex.main=0.5, cex.lab=0.6, cex.axis=0.4, las = 1)
  ...
  index <- which(date.list.20 %in% lockdown)
  title(paste(location,"NO2,", index.type, "as interruption,\n", as.character(format(date.list.19[[index-12]], "%b %d %Y")), "-", as.character(format(date.list.19[[index+11]], "%b %d %Y")), "vs", as.character(format((date.list.19[[index-12]] %m+% years(1)),"%b %d %Y")), "-", as.character(format((date.list.19[[index+11]] %m+% years(1)),"%b %d %Y"))), cex = 0.4)
...
plot.save.fxn <- function(location) {
  ...
  ggsave(paste0(location, ".schoolclosing.pdf"), path = "ITS_plots", plot1, width = 20, height = 10, units = "cm")
...
results.df.fxn <- function(model.list, index.titles, location) {
  ...
  #})
  sep.df <- purrr::map(sep.df, function(x) {x %>% .[c(8), ] %>% select(Predictors = term, p = p.value)
  })
  ...
  sep.df <- purrr::map(sep.df, ~rename_with(.x, stringr::str_replace, pattern = ".*Predictors", replacement = "Predictors"))
...
world.rural.df.list <- purrr::pmap(list("World", "rural", "whodeclaration", as.Date("2020-01-23")), NO2.df.filter.fxn)
...
sjPlot::tab_model(world.urban.model.list, terms = c("year2020:covid_lockdown", "no_of_weeks:year2020:covid_lockdown"), title = "ITS: Urban World Nitrogen Dioxide Concentrations During Lockdown, Significance of Different Lockdown Policies", show.ci = FALSE, dv.labels = paste("WHO declaration of pandemic")) #Table of p values. 
...
sjPlot::tab_model(world.rural.model.list, terms = c("year2020:covid_lockdown", "no_of_weeks:year2020:covid_lockdown"), title = "ITS: Rural World Nitrogen Dioxide Concentrations During Lockdown, Significance of Different Lockdown Policies", show.ci = FALSE, dv.labels = paste("WHO declaration of pandemic")) #Table of p values. 
...
#Saves plots as PDFs in urban/rural pairs. 
world.plot.save.fxn <- function(location) {
  plot8 <- plot_grid(urban.plots[[1]], rural.plots[[1]], ncol = 2, labels = c('A', 'B'))
  ggsave(paste0(location, ".restricttravel.pdf"), path = "ITS_plots", plot8, width = 17, height = 10, units = "cm")
}

world.plot.print.fxn <- function() {
  plot8 <- plot_grid(urban.plots[[1]], rural.plots[[1]], ncol = 2, labels = c('A', 'B'))
...
world.plot.save.fxn("World")
world.plot.print.fxn()
```
...
```{r,fig.width = 6, fig.height = 4,  eval = TRUE}
#Urban and rural data frame lists. 
japan.urban.df.list <- purrr::pmap(list("Japan", "urban", file.index.list[c(1:3, 6:8)], stringencyindex.split.country.df[[6]]$lockdown.date), NO2.df.filter.fxn)
japan.rural.df.list <- purrr::pmap(list("Japan", "rural", file.index.list[c(1:3, 6:8)], stringencyindex.split.country.df[[6]]$lockdown.date), NO2.df.filter.fxn)

#Scatter plots
purrr::pmap(list(japan.urban.df.list, 10E-5, "Urban Japan", index.title.list[c(1:3, 6:8)]), scatter_gplot)
purrr::pmap(list(japan.rural.df.list, 10E-5, "Rural Japan", index.title.list[c(1:3, 6:8)]), scatter_gplot)
```
...
sjPlot::tab_model(japan.urban.model.list, terms = c("year2020:covid_lockdown", "no_of_weeks:year2020:covid_lockdown"), title = "ITS: Urban Japan Nitrogen Dioxide Concentrations During Lockdown, Significance of Different Lockdown Policies", show.ci = FALSE, dv.labels = paste(index.title.list[c(1:3, 6:8)])) #Table of p values. 
...
sjPlot::tab_model(japan.rural.model.list, terms = c("year2020:covid_lockdown", "no_of_weeks:year2020:covid_lockdown"), title = "ITS: Rural Japan Nitrogen Dioxide Concentrations During Lockdown, Significance of Different Lockdown Policies", show.ci = FALSE, dv.labels = paste(index.title.list[c(1:3, 6:8)])) #Table of p values. 

#Plot
urban.plot.fxn <- function(country, n) {
  function() {
  period_scatter_plot(japan.urban.df.list[[n]], ylim=10E-5, paste("Urban", country), stringencyindex.split.country.df[[6]]$lockdown.date[[n]], index.title.list[c(1:3, 6:8)][[n]], japan.urban.model.list[[n]])
  }
}
urban.plots <- purrr::pmap(list("Japan", 1:6), urban.plot.fxn)

rural.plot.fxn <- function(country, n) {
  function() {
  period_scatter_plot(japan.rural.df.list[[n]], ylim=10E-5, paste("Rural", country), stringencyindex.split.country.df[[6]]$lockdown.date[[n]], index.title.list[c(1:3, 6:8)][[n]], japan.rural.model.list[[n]])
  }
}
rural.plots <- purrr::pmap(list("Japan", 1:6), rural.plot.fxn)
...
```{r,fig.width = 6, fig.height = 4, eval = TRUE}
purrr::pmap(list(japan.urban.df.list, "urban Japan", index.title.list[c(1:3, 6:8)]), res.fxn) #Residuals plots. 
purrr::pmap(list(japan.rural.df.list, "rural Japan", index.title.list[c(1:3, 6:8)]), res.fxn)
```
...
japan.urban.results <- results.df.fxn(japan.urban.model.list, index.title.list[c(1:3, 6:8)], "Japan")
japan.rural.results <- results.df.fxn(japan.rural.model.list, index.title.list[c(1:3, 6:8)], "Japan")
...
#results.avg.NO2 <- df.top3$Mean_Urban_NO2 %>% rep(., each = 2)
results.avg.NO2 <- df.top3$Mean_Urban_NO2 
...
rural.results.df <- cbind(rural.results.df, `Average Urban NO2 (mol/m^2)` = results.avg.NO2)
rural.results.df <- rural.results.df %>% relocate(`Average Urban NO2 (mol/m^2)`, .after = Predictors)
rural.results.df[, 4:12] <- round(rural.results.df[, 4:12], digits = 3)
rural.results.df[, 3] <- scales::scientific(rural.results.df[, 3], digits = 3)

table.fxn <- function(data, type, location) {
gt_tbl <- data %>% gt(rowname_col = "Location") 

gt_tbl.mod <- 
  gt_tbl %>%
  tab_header(
    title = md(paste0("**", location, " Nitrogen Dioxide Concentrations During Lockdown**")),
    #main=bquote(.(monitor)~"EPA Monitor vs Sentinel Satellite"~NO[2]))
    subtitle = md("Significance of Varying Lockdown Measures on Ambient NO2")) %>% 
  tab_row_group(
    label = md(html("**Sub-Saharan<br>Africa**")), 
    rows = 19:21) %>%
  tab_row_group(
    label = md(html("**Southeast Asia, East<br>Asia, and Oceania**")), 
    rows = 16:18) %>%
  tab_row_group(
    label = md("**South Asia**"), 
    rows = 13:15) %>%
  tab_row_group(
    label = md(html("**North Africa and<br>the Middle East**")), 
    rows = 10:12) %>%
  tab_row_group(
    label = md(html("**Latin America<br>and Caribbean**")), 
    rows = 7:9) %>%
  tab_row_group(
    label = md("**High-income**"), 
    rows = 4:6) %>%
  tab_row_group(
    label = md(html("**Central Europe,<br>Eastern Europe,<br>and Central Asia**")), 
    rows = 1:3) %>% 
  tab_spanner(
    label = md("*p* value"),
    columns = 4:12) %>% 
  cols_label(
    Predictors = md(html("**Predictors**")),
  	`Average Urban NO2 (mol/m^2)`	= md(html("**Average Urban<br>NO2 (mol/m^2)**")), 
  	`School Closures` = md(html("**School<br>Closures**")), 
    `Workplace Closures` = md(html("**Workplace<br>Closures**")), 
    `Cancelled Public Events` = md(html("**Cancelled<br>Public Events**")), 
    `Restrictions on Gatherings` = md(html("**Restrictions<br>on Gatherings**")),
    `Public Transport Closures` = md(html("**Public<br>Transport<br>Closures**")), 
    `Stay at Home Orders` = md(html("**Stay at<br>Home Orders**")),
    `Internal Travel Restrictions` = md(html("**Internal Travel<br>Restrictions**")),
    `International Travel Restrictions` = md(html("**International<br>Travel<br>Restrictions**")),
    `Overall Lockdown Stringency Index` = md(html("**Overall Lockdown<br>Stringency Index**"))
  ) %>%
  cols_align(
    align = "center")
  ...
  tab_source_note(
    source_note = md("Missing *p* values signify that the index date either falls outside the study period, the type of lockdown never occurred, or there is no date in data set."))%>%
  gtsave(filename= paste0(as.character(type), ".html"))
}
table.fxn(urban.results.df, "urban.results", "Urban")
table.fxn(rural.results.df, "rural.results", "Rural")
```
